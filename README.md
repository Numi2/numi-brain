...
A bounded Apple M4 correctness probe of the v0.12 immutable-parameter path is checked into [evidence/tissue-v0.12](evidence/tissue-v0.12/README.md). Mac mini M4 Pro qualification is deferred while an unrelated crow workload owns that GPU; the exact source commit is already synchronized to its clean checkout. The preceding two-device receptor-interrupt qualification remains in [evidence/tissue-v0.11](evidence/tissue-v0.11/README.md). These are correctness and visual-inspection artifacts, not production performance qualifications.
