...
> Status: formal architecture plus an executable heterogeneous mesoscale neural-tissue slice, deterministic scheduler oracle, and integrated Metal recurrent regional-token path with compiled sparse routes. The complete NumiBrain runtime is not implemented or qualified yet.
...
The executable also supports `--backend cpu` as a deterministic FP32 tissue oracle, `--structure homogeneous`, `--delays instantaneous`, and `--connectome none` as baselines. Its runtime input is an immutable schedule of timestamped receptor events rather than a shader-special-cased stimulus. `--stimulus-noise`, `--seed`, `--environment-id`, and `--episode-id` address bounded counter-random samples without mutable RNG state. The Metal path advances the compiled eight-module schedule once per root transaction, then consumes the private due list through a 10,752-scalar recurrent token operator with seven fixed sparse routes on the same encoder. Scheduler clocks, token state, and diagnostic records use private shadow generations that publish with tissue commit. Layered tissue structure, delay classes, both sparse graphs, event noise, lesion controls, scheduler roles, regional topology, and initial parameters are synthetic execution fixtures, not anatomical, conduction-velocity, receptor, injury, learned-model, or biological-timing calibration. Schema-v8 JSON evidence records program identity, token and route memory, scheduler and regional dispatches, CPU parity, replay, rollback, and tissue CPU–GPU error. PNG output is an inspection heatmap, not biological validation.

A reproducible Apple M4 development run of the transactionally integrated tissue, scheduler, and recurrent regional-token path is checked into [evidence/tissue-v0.8](evidence/tissue-v0.8/README.md). Earlier artifacts remain in [evidence/tissue-v0.7](evidence/tissue-v0.7/README.md) and its predecessor directories. These are correctness and visual-inspection artifacts, not production performance qualifications.
...
The first executable vertical slice now establishes an FP32 excitatory/inhibitory tissue field plus a schedule-driven recurrent regional primitive: synthetic per-site heterogeneity, finite-time axonal relay and conduction history, lesionable short-range coupling, destination-major sparse delayed tissue projections, timestamped noisy receptor events, GPU event compaction, counter-based randomness, private scheduler clocks, 10,752 region-major token scalars, immutable factorized slow parameters, seven compiled sparse regional routes, matching CPU oracles, Metal 4 dispatch, and committed/root-shadow/candidate transactions. Each attempted substep compacts due receptor events before tissue execution; each accepted root compacts due modules before timestamp-synchronous regional token execution. It does not yet implement calibrated receptors or conduction velocity, dynamic top-k routing, regional route delays, dense tiled regional matrices, parallel cohort compaction, the complete 96-module graph, NumanX interop, learning, memory, or motor control.

The next runtime-foundation work is regional route-delay history and deterministic top-k route compaction, followed by indirect active-region dispatch, parallel prefix-sum event and module compaction for cohorts, published parameter versions, nested NumanX transaction interop, and private-heap state storage. Later phases add calibrated causal receptor adapters, body schema, protective and motor systems, world modeling, motivation, skills and planning, memory, development, communication, and persistent life mode.
