...
The current same-process closed-loop artifact is in [evidence/numanx-joint-root-v0.4](evidence/numanx-joint-root-v0.4/README.md), with the full-muscle control retained in [v0.3](evidence/numanx-joint-root-v0.3/README.md) and the first six-muscle proof in [v0.2](evidence/numanx-joint-root-v0.2/README.md). On Apple M4, one exact private 416-channel NumiBrain excitation buffer drove the complete NumanX MyoSim muscle catalog and articulated state. An immutable cross-language attachment catalog localized the passive-load event from tendon `215` to route bodies `41 -> 34` and the commanded tendon `3` to `16 -> 1`; commanded activation and force rose from a zero-activation start, one rejected physical candidate replayed exactly, and fresh processes emitted byte-identical committed JSON. Only the first six channels currently have fixture protective gains; the other 410 have zero neural output gain but may still generate passive physical force. This is bounded correctness evidence: NumanX force evaluation is on Metal, but the reference bridge synchronizes between separate queues, stages diagnostics, integrates the Core body on the CPU, and publishes the two roots sequentially.
...
The next runtime-foundation work uses the endpoint attachment graph for localized overload representation and protective-muscle selection, then adds stable body-side and anatomical labels plus any needed intermediate route bodies. Articulated integration and sensor extraction must move onto the shared GPU timeline, and sequential root publication must become a fail-closed coordinated publication boundary. Incremental fast regional generations can replace the current bounded canonical-prefix recomputation after exact parity is retained. GPU-native plan construction, immutable successor-buffer activation across runtimes, persistent checkpoints, and private-heap state storage follow. Later phases add calibrated causal receptors, body schema, voluntary and learned motor systems, world modeling, motivation, skills and planning, memory, development, communication, and persistent life mode.
