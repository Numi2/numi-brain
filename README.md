...
The current same-process closed-loop artifact is in [evidence/numanx-joint-root-v0.5](evidence/numanx-joint-root-v0.5/README.md), with the attachment-catalog control retained in [v0.4](evidence/numanx-joint-root-v0.4/README.md), the full-muscle control in [v0.3](evidence/numanx-joint-root-v0.3/README.md), and the first six-muscle proof in [v0.2](evidence/numanx-joint-root-v0.2/README.md). On Apple M4, one exact private 416-channel NumiBrain excitation buffer drove the complete NumanX MyoSim muscle catalog and articulated state. An immutable cross-language attachment catalog localized the passive-load event from tendon `215` to route bodies `41 -> 34` and the commanded tendon `3` to `16 -> 1`; the accepted overload is now retained as transaction-owned localized feedback only after root commit. Commanded activation and force rose from a zero-activation start, one rejected physical candidate replayed exactly, and fresh processes emitted byte-identical committed JSON. Only the first six channels currently have fixture protective gains; the other 410 have zero neural output gain but may still generate passive physical force. This is bounded correctness evidence: the localized list is a Swift sidecar rather than a GPU body-schema field, and the reference bridge synchronizes between separate queues, stages diagnostics, integrates the Core body on the CPU, and publishes the two roots sequentially.
