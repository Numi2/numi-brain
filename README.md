...
The first same-process closed-loop artifact is in [evidence/numanx-joint-root-v0.2](evidence/numanx-joint-root-v0.2/README.md). On Apple M4, the actual private NumiBrain excitation buffer drove NumanX MyoSim force and articulated state, an accepted tendon-local muscle-load event changed the next neural output, one rejected physical candidate replayed exactly, and two fresh processes emitted identical committed JSON. This is bounded correctness evidence: NumanX force evaluation is on Metal, but the reference bridge synchronizes between separate queues, stages diagnostics, integrates the Core body on the CPU, and publishes the two roots sequentially.
