...
The v0.15 dispatch executable preserves independent version-bound scheduler shadows, compiles their active module work into canonical timestamp/module groups, materializes the flattened plan into private Metal 4 buffers, and launches two consumers from GPU-generated indirect arguments:
...
The first consumer emits exact auditable work records. The second advances a distinct environment-major compact recurrent-state generation for each active agent from deterministic nonzero input state. The executable verifies retry and discarded-shadow identity, canonical input ordering, exact GPU materialization and work consumption, FP32 CPU-reference state parity, exact counters and timestamps, interrupt isolation, exact replay fingerprints, and stale-parameter rejection. The CPU currently owns plan construction; Metal owns private region-major materialization and the no-readback handoff into both consumers. This compact 32-byte-per-module state is diagnostic; it is not the authoritative 10,752-scalar regional-token state, GPU prefix-sum grouping, or cohort throughput qualification.

The latest qualified 8,192-environment Apple M4 artifact will be recorded under `evidence/cohort-dispatch-v0.15` after the shared Mini GPU is free. The exact v0.14 indirect-consumption artifact remains in [evidence/cohort-dispatch-v0.14](evidence/cohort-dispatch-v0.14/README.md), with the preceding materialization-only artifact in [v0.13](evidence/cohort-dispatch-v0.13/README.md). Command-feedback timings cover only the named kernels and are not production performance claims.
