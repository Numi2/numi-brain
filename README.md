...
The current same-process closed-loop artifact is in [evidence/numanx-joint-root-v0.6](evidence/numanx-joint-root-v0.6/README.md), with committed-localization v0.5, attachment-catalog v0.4, full-muscle v0.3, and selected-muscle v0.2 retained as controls. On Apple M4, one exact private 416-channel NumiBrain excitation buffer drove the complete NumanX MyoSim muscle catalog and articulated state. Accepted tendon `215` load becomes a fingerprinted body-load frame on route bodies `34` and `41`, a 37-muscle endpoint-sharing neighborhood, and a Metal source-inhibition mask that zeros tendon `215` in the following protective output; tendon `215` has zero fixture neural gain and passive force, so this proves command inhibition rather than physical load reduction. Commanded tendon `3` activation and force rose from a zero-activation start, one rejected physical candidate replayed exactly, and fresh processes emitted byte-identical committed JSON. The body frame and neighborhood remain Swift sidecars, and the reference bridge synchronizes separate queues, stages diagnostics, integrates the Core body on the CPU, and publishes the two roots sequentially.
