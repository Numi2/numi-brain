...
The v0.16 dispatch executable preserves independent version-bound scheduler shadows, compiles their active module work into canonical timestamp/module groups, materializes the flattened plan into private Metal 4 buffers, and launches three consumers from GPU-generated indirect arguments:
...
The first consumer emits exact auditable work records. The second advances a distinct environment-major compact diagnostic-state generation for each active agent. The third launches one 64-lane threadgroup per agent and advances an independent authoritative 10,752-scalar recurrent token generation from deterministic nonzero input state and immutable factorized parameters. The executable verifies retry and discarded-shadow identity, canonical input ordering, exact GPU materialization and work consumption, FP32 CPU-reference diagnostic and token parity, exact counters and timestamps, interrupt isolation, full-cohort state ownership, and exact replay fingerprints. The CPU currently owns plan construction; Metal owns private region-major materialization and the no-readback handoff into all three consumers. Long-range routes are intentionally disabled in this cohort slice until route history and routing state become per-agent transactional generations. This is not GPU prefix-sum grouping or cohort throughput qualification.

The latest v0.16 cohort-token artifact will be recorded after the shared Mini GPU is free; its cohort size will be selected against the much larger token generations rather than assumed to match the lightweight v0.14 work-record probe. The exact 8,192-environment v0.14 indirect-consumption artifact remains in [evidence/cohort-dispatch-v0.14](evidence/cohort-dispatch-v0.14/README.md), with the preceding materialization-only artifact in [v0.13](evidence/cohort-dispatch-v0.13/README.md). Command-feedback timings cover only the named kernels and are not production performance claims.
