...
> Status: formal architecture plus an executable heterogeneous mesoscale neural-tissue slice, deterministic scheduler oracle, a compiled NumanX joint-transaction contract, and a versioned Metal cohort executor with GPU-generated indirect work, independent authoritative recurrent token state, delayed sparse-route history, and dynamic routing state per agent. The complete NumiBrain runtime is not implemented or qualified yet.
