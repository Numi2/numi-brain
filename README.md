...
> Status: formal architecture plus an executable heterogeneous mesoscale neural-tissue vertical slice, deterministic scheduler oracle, and integrated Metal due-selection and compact regional-state path. The complete NumiBrain runtime is not implemented or qualified yet.
...
The executable also supports `--backend cpu` as a deterministic FP32 oracle, `--structure homogeneous`, `--delays instantaneous`, and `--connectome none` as baselines. Its runtime input is now an immutable schedule of timestamped receptor events rather than a shader-special-cased stimulus. `--stimulus-noise`, `--seed`, `--environment-id`, and `--episode-id` address bounded counter-random samples without mutable RNG state. The Metal path advances the compiled eight-module schedule once per root transaction, then consumes the private due list through an eight-lane regional population-state operator on the same encoder. Scheduler clocks and regional states use private shadow generations that publish with tissue commit. Layered structure, delay classes, the bilateral sparse graph, event noise, lesion controls, scheduler roles, and compact regional traces are synthetic experiment inputs, not anatomical, conduction-velocity, receptor, injury, or biological-timing calibration. Schema-v7 JSON evidence records scheduler and regional dispatches, ABI memory, committed generations, CPU parity, replay, rollback, and tissue CPU–GPU error. PNG output is an inspection heatmap, not biological validation.

A reproducible Apple M4 development run of the transactionally integrated tissue, scheduler, and compact regional-state path is checked into [evidence/tissue-v0.7](evidence/tissue-v0.7/README.md). Earlier artifacts remain in [evidence/tissue-v0.6](evidence/tissue-v0.6/README.md), [evidence/tissue-v0.5](evidence/tissue-v0.5/README.md), [evidence/tissue-v0.4](evidence/tissue-v0.4/README.md), [evidence/tissue-v0.3](evidence/tissue-v0.3/README.md), [evidence/tissue-v0.2](evidence/tissue-v0.2/README.md), and [evidence/tissue-v0.1](evidence/tissue-v0.1/README.md). These are correctness and visual-inspection artifacts, not production performance qualifications.
...
This executable remains the standalone CPU oracle for multi-agent semantic checks. The tissue executable now runs bounded one-agent due selection and a compact eight-module regional trace operator inside its Metal command timeline. Neither path is the final trainable 96-module token graph or large-cohort scheduler qualification.
...
The first executable vertical slice now establishes an FP32 excitatory/inhibitory tissue field plus a compact schedule-driven regional primitive: synthetic per-site heterogeneity, a finite-time axonal relay, explicit conduction history, lesionable short-range coupling, destination-major sparse delayed projections, timestamped noisy receptor events, bounded GPU event compaction, counter-based randomness, adaptation, private scheduler clocks, private regional state generations, CPU oracles, Metal 4 dispatch, and committed/root-shadow/candidate transactions. Each attempted substep compacts due event indices before tissue execution; each accepted root compacts due modules before the regional operator consumes them. It does not yet implement spiking neurons, calibrated receptors or conduction velocity, parallel cohort-scale compaction, the trainable anatomical multi-region token graph, NumanX interop, learning, memory, or motor control.

The next runtime-foundation work is replacing the compact regional trace with the trainable token-state operator and sparse routed inputs, then adding indirect active-region dispatch, parallel prefix-sum event and module compaction for large cohorts, immutable parameter versions, nested NumanX transaction interop, and private-heap state storage. Later phases add calibrated causal receptor adapters, body schema, protective and motor systems, world modeling, motivation, skills and planning, memory, development, communication, and persistent life mode.
