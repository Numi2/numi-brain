...
final class WatchdogLifecycleProtocolTests: XCTestCase {
  ...
  private let hash = String(repeating: "d", count: 64)
  ...
  func testStopAlwaysOutranksReadinessAndCompletion() throws {
    ...
    let completion = try WatchdogOwnerCompletion(arm: a, lastSettledHeartbeat: heartbeat(),
      completedMonotonicNanoseconds: 160, terminalEvidenceArtifactSHA256: hash)
  ...
  func testOwnerLifecycleRequiresArmAndIdleSettledCompletion() throws {
    try withDirectory { directory in
      ...
      XCTAssertThrowsError(try lifecycle.complete(terminalEvidenceArtifactSHA256: hash, nowNanoseconds: 140))
      let permit = try owner.beginRoot(nowNanoseconds: 140)
      XCTAssertThrowsError(try lifecycle.complete(terminalEvidenceArtifactSHA256: hash, nowNanoseconds: 145))
      try owner.recordSettledRoot(permit, publicGeneration: 1, transactionFingerprint: 10,
        settledMonotonicNanoseconds: 150, terminalEvidenceArtifactSHA256: hash)
      let completion = try lifecycle.complete(terminalEvidenceArtifactSHA256: hash, nowNanoseconds: 160)
      XCTAssertEqual(try WatchdogFileProtocol.readOwnerCompletionIfPresent(completionURL), completion)
      XCTAssertThrowsError(try lifecycle.complete(terminalEvidenceArtifactSHA256: hash, nowNanoseconds: 170))
    }
  }
