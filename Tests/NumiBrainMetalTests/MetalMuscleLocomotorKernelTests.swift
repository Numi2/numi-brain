...
final class MetalMuscleLocomotorKernelTests: XCTestCase {
  ...
  func testSpindleFeedbackAlternationBoundsAndMissingEvidence() throws {
    ...
    for sign: Float in [1, -1] {
      rows += [0, 1, 0, 3]
      rows += [Float(0.25), 0.2, 0.5, 0.02, sign * 0.1, 0, 0.4, 0].map(\.bitPattern)
    }
    ...
    func evaluate(length: Float = 0.25, velocity: Float = 0, phase: Float = 0, valid: UInt32 = 3) throws -> [Float] {
      ...
      let output = try upload([Float(-1), -1])
      let uniforms = try upload([UInt32(2), phase.bitPattern, 0, 0])
      ...
      return Array(UnsafeBufferPointer(start: output.contents().assumingMemoryBound(to: Float.self), count: 2)).map { tanh($0) }
    }
    let rest = try evaluate(); for value in rest { XCTAssertEqual(value, 0.2, accuracy: 1e-6) }
    ...
    XCTAssertEqual(try evaluate(length: 0.01), [0, 0])
    XCTAssertEqual(try evaluate(valid: 0), [0, 0])
    XCTAssertEqual(try evaluate(valid: 1), [0, 0])
    XCTAssertEqual(try evaluate(valid: 2), [0, 0])
    XCTAssertEqual(try evaluate(length: .nan), [0, 0])
    XCTAssertEqual(try evaluate(velocity: .infinity), [0, 0])
    XCTAssertEqual(try evaluate(length: .greatestFiniteMagnitude), [0, 0])
