...
The source and deterministic tests are now joined by a committed attachment-localized full-muscle execution. At NumiBrain `be07275` and isolated NumanX `5cdee51`, one process reconstructed the exact ordered 416-muscle, 157-body attachment catalog with the same fingerprint in Swift and C++, passed the private NumiBrain allocation into the NumanX MyoSim command buffer, started all activations at zero, integrated the full-body Core candidate, bound the accepted passive-load peak at tendon `215` and route bodies `41 -> 34` to its exact physical token and catalog, published that localized observation only after root commit, changed the next maximum excitation from `0.05` to `0.52`, localized commanded tendon `3` to `16 -> 1`, raised its activation and force across accepted candidates, replayed a rejected physical candidate exactly, and committed three accepted substeps. Fresh Apple M4 processes emitted byte-identical JSON; the artifact is in [`evidence/numanx-joint-root-v0.5`](evidence/numanx-joint-root-v0.5/README.md), with v0.4, v0.3, and v0.2 retained as controls. The committed list remains a Swift sidecar rather than a GPU body-schema field. The reference still synchronizes separate Metal queues, stages MyoSim diagnostics, integrates articulated state on the CPU, and publishes brain and physical roots sequentially. Route order is mechanical identity, not anatomical proximal/distal or body-side interpretation. Only the first six channels use fixture gains; the remaining 410 preserve real source-tendon order with zero neural output gain while retaining their NumanX passive force.
...
The XCTest suite contains 83 passing tests locally: 19 tissue and timestamped-history CPU tests, 37 scheduler/regional/parameter/dispatch/joint/motor/catalog CPU tests, and 27 Metal 4 tests. Golden vectors pin the random, module, protective-command, muscle-profile, motor-output, and attachment-catalog fingerprints. Tissue tests require physical-microsecond delay sampling, deterministic interpolation, fail-closed coverage loss, and exact variable-duration retry, abort, and chunking identity. Parameter, dispatch, joint, motor, and catalog tests require canonical compiled fingerprints, serialization tamper rejection, accepted-only history, accepted-force receptor transduction, exact NumanX source-tendon and endpoint-body order, bounded endpoint coordinates, canonical protective-drive validation, unique muscle identities, exact fused gain mapping, and bounded excitation output. Metal tests require corrected-duration CPU parity, exact physical timestamp ownership, accepted fast-prefix response, CPU-identical protective command and muscle-output derivation, transaction-bound NumanX address/shape validation, rejected-prefix/command/output silence, immutable version identity, recurrent token and route CPU parity, dynamic routing causality, interactive-versus-batched joint parity, and joint retry, abort, replay, and chunking equivalence.
