...
- Current state: specification, GPU-compacted tissue source slice v0.14, scheduler CPU oracle v0.1, immutable parameter-manifest/publication boundary v0.12, versioned cohort-dispatch boundary v0.19 with GPU-generated environment-major invocations plus per-agent routed token, delayed-history, and routing-state generations, and integrated Metal scheduler/regional path v0.8
...
- a private GPU-generated canonical invocation span per active environment, field-wise fingerprinted and checked exactly against the CPU plan before evidence publication;
