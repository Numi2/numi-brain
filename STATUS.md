...
- Current state: specification, tissue source slice v0.19 with variable-duration physical-time CPU and Metal relay paths, scheduler CPU oracle v0.1, immutable parameter-manifest/publication boundary v0.12, compiled NumanX joint-transaction contract v0.9 with accepted-only fast regional prefixes, corrected-duration interactive Metal tissue candidates, GPU-resident protective command and per-muscle output, and a transaction-bound NumanX motor-candidate packet, versioned cohort-dispatch boundary v0.20 with parallel GPU environment-major invocation compaction plus per-agent routed token, delayed-history, and routing-state generations, and integrated Metal scheduler/regional/protective-motor path v0.12
...
The compiled v0.9 handoff contract currently proves:
