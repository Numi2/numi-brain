...
- Current state: specification, GPU-compacted tissue source slice v0.14, scheduler CPU oracle v0.1, immutable parameter-manifest/publication boundary v0.12, compiled NumanX joint-transaction contract v0.4 with accepted-only events and interactive Metal tissue candidates, versioned cohort-dispatch boundary v0.20 with parallel GPU environment-major invocation compaction plus per-agent routed token, delayed-history, and routing-state generations, and integrated Metal scheduler/regional path v0.8
