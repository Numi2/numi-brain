...
- Current state: specification, tissue source slice v0.15 with a variable-duration physical-time CPU relay oracle and fixed-duration Metal v0.14 path, scheduler CPU oracle v0.1, immutable parameter-manifest/publication boundary v0.12, compiled NumanX joint-transaction contract v0.4 with accepted-only events and interactive Metal tissue candidates, versioned cohort-dispatch boundary v0.20 with parallel GPU environment-major invocation compaction plus per-agent routed token, delayed-history, and routing-state generations, and integrated Metal scheduler/regional path v0.8
...
The compiled v0.4 handoff contract currently proves:
...
This is a source and deterministic-test claim. NumanX has not yet returned or atomically published one of these physical-state records. The reference path synchronizes after each Metal candidate, defers accepted fast events to root finalization, and rejects corrected retry durations until conduction history is indexed by physical time.
