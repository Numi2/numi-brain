...
- Affective-state implementation slice: a 64-byte per-agent Metal state derives transient pain, pleasure, and relief from accepted fresh evidence and is carried in the v12 1,152-byte committed-transition ABI. Species-template format v9 fingerprints the optional interoception feature schema; the typed `NumanXFullBodyV1` mapping covers the six documented normalized fields in the 416-by-6 profile, while opaque profiles are not interpreted by feature position. Affect is wired into attention/workspace, memory salience, candidate scoring, and committed-transition learning emphasis; physical-time decay is applied in state updates and candidate scoring. The selected, validated affect configuration now controls Metal source weights, gains, evidence-age gates, and decay constants, and its fingerprint is bound to arena layout and checkpoint compatibility. These are implementation details, not a separate reward authority.
- Episodic affect persistence: Core `EpisodicRecord` format 2 and Metal episode format 3 carry eight affect/source floats, source-validity mask, and timestamp. The Metal memory-record layout is v19; workspace metadata is 112 bytes, the hot accumulator is allocated at a 320-byte stride, warm episode records use a 192-byte stride, and the archive-page payload is format 2. Metal retrieval publishes the affect values into workspace content and validity/timestamp into workspace metadata; MLX replay format 14 validates and exposes those fields. Older Core, Metal, warm-record, and archive-page formats fail closed. A forced accepted-root test checks journal creation, warm compression, archive promotion and paging, and retrieval from the archive. See [`docs/AFFECTIVE_STATE.md`](docs/AFFECTIVE_STATE.md).
- Affective evidence boundary: typed-schema CPU/Metal parity uses a synthetic one-receptor fixture, including signed negative temperature deviation; a separate synthetic 416-receptor Metal test checks aggregation and incomplete-coverage rejection. Focused tests cover invalid evidence, matched attention, restorative option value, committed learner input, persistent episode memory, accepted recovery gains, evidence-age rejection, planner decay, checkpoint continuation and cross-configuration restore rejection, rejected-evidence rollback, and emergency-stop behavior under maximal pleasure. These are software checks, not native behavioral, biological, physiological-calibration, or physical-safety qualification.
- Matched affect controls and native attempt: the explicit fingerprinted disabled configuration neutralizes derived affect and its consumers while leaving raw sensory paths, reflexes, emergency stops, and separate pain-cost factors active. Capture-run format 5 retains native world/physics identity and bridge-validated prepared-state fingerprints; historical formats 2/3/4 remain readable. Frozen protocol v2 was attempted on the M4 Pro Mac mini with matched world and prepared initial state, seed, and task. Both arms accepted 103 roots, then rejected step 104 at the Human support-contact gate. Isolated diagnostic replays identify the same invalid normal impulse (`-0.000127762556`) on support row 12, the `foot_col3_l` capsule on the left calcaneus. The diagnostic disabled prefix differs from its original at one step-30 packet fingerprint, so the diagnostic pair does not establish exact matched provenance. Neither replay reached the 10,002-root horizon or produced an eligible behavior comparison. The failed pair, diagnostic logs, source mapping, run hashes, and retained inputs are in [`evidence/affect-native-pair-20260920`](evidence/affect-native-pair-20260920/README.md). No behavioral benefit is demonstrated; physiological calibration, biological validity, and physical safety remain unestablished.
- Gate C packaging and capture: a deterministic version-2 `.nbpolicy` binds a learned publication's exact bytes to separate model/runtime/MetalRobo-owner/somatic-controller/safety identities, source and split provenance, action-generation/latency declarations, calibrated executable uncertainty thresholds, and the complete Gate C evidence-axis manifest. Version-4 qualification evidence is produced only after authoritative accepted/rejected release. The production `numi-brain-gate-c` runner content-addresses exact settled full-body sensors, terminal roots, all nine immutable learner sections, and the exact native timestep; their live Metal fingerprints are independently recomputed. New roots additionally retain complete committed memory batches before/after the root and an identity-free semantic motor artifact, with verified root-to-root continuity, accepted advance, and rejected no-mutation. The original 1-millisecond +0.5-degree scene remains an honest `2/3` failure with authoritative root-4 rejection. At 100 microseconds, the learned candidate passes its predeclared 20 ms p99 latency budget over 100 roots; paired baseline/learned held-out cohorts each complete 10/10 post-bootstrap observations, with zero learned advantage, and a semantic replay is exact across 22 roots. A separate local 20-ID/20-all-invalid cohort yields AUROC 1.0, supervision-or-reject recall 1.0, unsafe acceptance 0, and 20 authoritative prepublication rejections with no generation advance. A separate same-root emergency cohort challenges all 40 roots, including 20 uncertainty-rejected roots, and recomputes zero protective bypasses and zero safety violations while accepted roots retain nonzero learned drive. The fixed local long-horizon suite now recomputes `10/10` for state aliasing, interrupted-task rejection/recovery, and four-root delayed consequences, with exact semantic sensor/action and complete committed-memory evidence. Absolute species-critical rules now read raw validity-gated receptors, so learned sensory bias cannot fabricate a vital interrupt; a real two-root M4 regression covers the former healthy-interoception emergency. A fresh coordinate-exact 8-example MLX adaptation finishes in about 1.06 seconds and retains prior success, but remains `0.4 -> 0.4`; it materially changes all 416 commands without changing the 1.1-millisecond physical trajectory, and a four-example pre-failure calibration also fails. These are local non-promotable development results; complete independent partitions, independent-distribution OOD and safety calibration, a task-conditioned positive few-shot result at a meaningful physical horizon, cross-task/object/body qualification, and independent embodiment evidence remain absent.
- Gate C head-posture learning: the contact-independent body-23 objective now binds accepted vestibular/action response into a deterministic MLX successor. An exploratory bounded gain increase changed 104 muscle commands but first produced zero lift advantage, then a stronger probe produced a harmful `-0.23841858` micrometre delta. That exact negative evaluation is now a transitive causal calibration for gain direction `-1`; missing, zero, mismatched, and recursively calibrated probes fail closed. The calibrated candidate completed 100/100 roots on a third fresh scene and changed mean command `0.00598 -> 0.01088`, but retained exactly zero head-lift advantage. A separately predeclared fourth-scene Apple M4 comparison then completed 1,000/1,000 accepted roots for both baseline and candidate over 100 milliseconds. It produced a directionally correct `+0.11920929` micrometre head-relative advantage, only about 12 percent of the unchanged one-micrometre minimum; evaluation `961066d605aa89efa4f8de364ba3e2d35ba536cbfdab9baebc2a6074a4b4cbc0` independently recomputes and remains non-promotable. One subsequent predeclared stronger successor increased calibrated learning rate `4 -> 12` while retaining objective weight `4` and the existing `+/-4` parameter bound; training run `933ed5614e07bcbd8a51d2fadce887b5fff47b5b9780ae4a4f00979570f6ff2a` emitted candidate `c60c18f708a81310a1caf396066af10eb98799eb602992f289ac6a5d7b115314`. On a fifth fresh scene, baseline and candidate again completed 1,000/1,000 accepted roots, but both measured exactly `-0.00029051304 m` head-relative lift. Evaluation `11d2c107be2eb59fbceed6205d101bace5fee8e37a345928cbaabe31c591a472` independently recomputes the exact `0 m` advantage. Gate C remains open pending a newly justified fresh-scene positive physical result, plus the existing independent-partition and constrained-dynamics requirements; threshold relaxation and post-hoc sweeps remain excluded.
- Gate C anatomical liveness diagnostic: accepted identification attempts now advance even when a 10-microsecond endpoint delta is below resolution, and actuator-model residuals no longer become whole-body damage authority. Matched 20-root Apple M4 runs retained 20/20 accepted roots; the body-23 goal advanced once each through actuators `22, 23, 32...42` and caused differences only in proprioceptive and muscle-receptor payloads from root 3 onward. Exact vestibular root-relative head height remained unchanged. This closes the repeated-probe liveness fault but is only a 200-microsecond causal diagnostic; it does not change the qualified 100-microsecond default or close Gate C.
- Full-body source provenance: the native 129/128 runtime now fingerprints the exact immutable NHRIGID2, NHMYO, and NHCNT byte streams with domain separation and mixes that identity into HumanIO program/publication authority. The focused Apple M4 prepared-root probe independently recomputes the composite fingerprint and proves that a one-bit NHCNT mutation changes model identity. This closes payload-identity drift; it does not implement support contact.
...
The committed-transition ABI used for this Gate B checkpoint was version 11;
it records the exact sixteen accepted somatic-synergy coordinates. The Metal
decision runtime evaluates the learned head against a deterministic
validity-bound 24-coordinate raw-sensor
...
On the Apple M4 real full-body +0.5-degree held-out support task, independent
three-root interventions close the sensor-to-learned-action-to-416-muscle-to-
later-physical-outcome chain for all seven channels. The task-trained successor
comes from accepted -2/+2-degree worlds with nonzero actions rather than a
passive zero-action cohort. Learned-action deltas range from `0.0011418909` to
`0.29194874`; decoded muscle-command deltas range from `0.00005773455` to
`0.055146806`; and every intervention worsens an independently published
continuous outcome under the same accepted goal/option sequence. The native
interoceptive channel now carries six bounded local-physiology fields integrated
from causal MyoSim
...
tensor as nine body-wide scalars. The learned sketch preserves energy,
respiratory, thermal, fatigue, and damage direction; validity gates evidence
instead of becoming a motor feature, and closed-loop ablation invalidates the
channel rather than fabricating zero energy/oxygen. Intact interoception realizes
mean physical muscle activation `0.0075913453`, versus `0.0016865405` when
ablated, worsening normalized activation deficit `0.99240863 -> 0.9983135`.
The former active-acquisition gain has been
...
physical search-coverage criterion. The bounded Gate B causal-control matrix is
