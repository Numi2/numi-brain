...
- Affective-state implementation slice: a 64-byte per-agent Metal state derives transient pain, pleasure, and relief from accepted fresh evidence and is carried in the v12 1,152-byte committed-transition ABI. Species-template format v9 fingerprints the optional interoception feature schema; the typed `NumanXFullBodyV1` mapping covers the six documented normalized fields in the 416-by-6 profile, while opaque profiles are not interpreted by feature position. Affect is wired into attention/workspace, memory salience, candidate scoring, and committed-transition learning emphasis; physical-time decay is applied in state updates and candidate scoring. These are implementation details, not a separate reward authority.
- Affective evidence boundary: typed-schema CPU/Metal parity uses a synthetic one-receptor fixture; a separate synthetic 416-receptor Metal test checks aggregation and incomplete-coverage rejection. A synthetic regression also confirms that a fresh critical-physiology event still selects the protective reflex stop when stored pleasure is maximal. None is a native NumanX paired run or physical intervention. No paired native affect-enabled/control result, behavioral-benefit result, or physiological calibration has been produced; biological validity and physical safety remain unestablished. See [`docs/AFFECTIVE_STATE.md`](docs/AFFECTIVE_STATE.md).
