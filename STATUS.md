...
- Current state: specification, tissue source slice v0.16 with variable-duration physical-time CPU and Metal relay paths, scheduler CPU oracle v0.1, immutable parameter-manifest/publication boundary v0.12, compiled NumanX joint-transaction contract v0.5 with accepted-only events and corrected-duration interactive Metal tissue candidates, versioned cohort-dispatch boundary v0.20 with parallel GPU environment-major invocation compaction plus per-agent routed token, delayed-history, and routing-state generations, and integrated Metal scheduler/regional path v0.8
...
The compiled v0.5 handoff contract currently proves:
...
This is a source and deterministic-test claim. NumanX has not yet returned or atomically published one of these physical-state records. The reference path synchronizes after each Metal candidate and still defers accepted fast events to root finalization.
