...
The source and deterministic tests are now joined by an attachment-localized full-muscle execution. At NumiBrain `c0f2bf8` and isolated NumanX `5cdee51`, one process reconstructed the exact ordered 416-muscle, 157-body attachment catalog with the same fingerprint in Swift and C++, passed the private NumiBrain allocation into the NumanX MyoSim command buffer, started all activations at zero, integrated the full-body Core candidate, localized the passive-load peak at tendon `215` to route bodies `41 -> 34`, changed the next maximum excitation from `0.05` to `0.52`, localized commanded tendon `3` to `16 -> 1`, raised its activation and force across accepted candidates, replayed a rejected physical candidate exactly, and committed three accepted substeps. Fresh Apple M4 processes emitted byte-identical JSON; the artifact is in [`evidence/numanx-joint-root-v0.4`](evidence/numanx-joint-root-v0.4/README.md), with v0.3 and v0.2 retained as controls. The reference still synchronizes separate Metal queues, stages MyoSim diagnostics, integrates articulated state on the CPU, and publishes brain and physical roots sequentially. Route order is mechanical identity, not anatomical proximal/distal or body-side interpretation. Only the first six channels use fixture gains; the remaining 410 preserve real source-tendon order with zero neural output gain while retaining their NumanX passive force.
