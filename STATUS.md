...
- Affective evidence boundary: typed-schema CPU/Metal parity uses a synthetic one-receptor fixture; a separate synthetic 416-receptor Metal test checks aggregation and incomplete-coverage rejection. Focused local checks pass for matched attention, restorative option value, committed learner input, and episode-memory event selection; the memory test raises only its test fixture's segmentation threshold to inspect the accumulator before the normal salient-boundary journal clears it. Configuration-specific accepted updates, planner decay, same-config checkpoint continuation, cross-config restore rejection, and rejected-evidence rollback also pass targeted Metal tests; CPU affect tests pass 17/17 and the MLX committed-input test passes 1/1 after building the pinned MLX metallib. A synthetic regression also confirms that a fresh critical-physiology event still selects the protective reflex stop when stored pleasure is maximal. None is a native NumanX paired run or physical intervention. No paired native affect-enabled/control result, behavioral-benefit result, or physiological calibration has been produced; biological validity and physical safety remain unestablished. See [`docs/AFFECTIVE_STATE.md`](docs/AFFECTIVE_STATE.md).
- Matched affect controls: an explicit fingerprinted disabled configuration neutralizes only derived affect and its consumers, leaving raw sensory paths, reflexes, emergency stops, and separate pain-cost factors active. New root-sample format 2 tags synthetic bootstrap versus accepted native aggregate evidence; capture-run format 5 retains native world/physics identity and any bridge-validated prepared-state fingerprint while formats 2/3/4 remain readable. `study-affect` requires the same authored world and prepared initial state, compares fresh enabled/disabled traces, and records first accepted native aggregate samples. Required NumanX inputs are still incomplete, so no native pair or behavioral-benefit result exists.
