...
- Current state: specification, tissue source slice v0.19 with variable-duration physical-time CPU and Metal relay paths, scheduler CPU oracle v0.1, immutable parameter-manifest/publication boundary v0.12, compiled NumanX joint-transaction contract v0.10 with accepted-only fast regional prefixes, corrected-duration interactive Metal tissue candidates, GPU-resident protective command and per-muscle output, a transaction-bound NumanX motor-candidate packet, and lifetime-safe borrowing of the exact resident Metal buffers, versioned cohort-dispatch boundary v0.20 with parallel GPU environment-major invocation compaction plus per-agent routed token, delayed-history, and routing-state generations, and integrated Metal scheduler/regional/protective-motor path v0.13
...
This is a source and deterministic-test claim. The isolated NumanX branch at `b42b283` can consume a private borrowed excitation buffer in its owning MyoSim command buffer and the full-body M4 Pro probe measures a causal activation, force, and articulated-state response. NumiBrain and that receiver have not yet been invoked together in one executable, and NumanX has not returned or atomically published one of these physical-state records to NumiBrain. The reference path synchronizes after each Metal candidate. The six-channel NumiBrain profile remains synthetic and is not the 416-muscle body catalog used by the receiver probe.
