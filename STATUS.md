...
- Current state: specification, tissue source slice v0.17 with variable-duration physical-time CPU and Metal relay paths, scheduler CPU oracle v0.1, immutable parameter-manifest/publication boundary v0.12, compiled NumanX joint-transaction contract v0.6 with accepted-only fast regional prefixes and corrected-duration interactive Metal tissue candidates, versioned cohort-dispatch boundary v0.20 with parallel GPU environment-major invocation compaction plus per-agent routed token, delayed-history, and routing-state generations, and integrated Metal scheduler/regional path v0.9
...
- NumanX interop: compiled root, nested-substep, accepted-physics, and joint-commit token ABI plus a deterministic Swift coordinator, accepted-only receptor-event handoff, corrected-duration interactive Metal candidate advance/accept/reject/finalize APIs, accepted fast scheduler/regional prefixes between physical candidates, and a joint publication guard; no live NumanX adapter, protective motor output, or cross-runtime atomic pointer publication yet
...
- GPU performance evidence: bounded Apple M4 Pro v0.20 and Apple M4 v0.19 routed-cohort correctness, Apple M4 v0.14 lightweight indirect-consumption correctness, Apple M4 Pro interactive-joint v0.4 full-suite correctness, and Apple M4 Pro/M4 v0.12 and earlier tissue correctness probes only; production throughput and counter qualification remain pending
...
The compiled v0.6 handoff contract currently proves:
...
This is a source and deterministic-test claim. NumanX has not yet returned or atomically published one of these physical-state records. The reference path synchronizes after each Metal candidate; an accepted interrupt can affect the next candidate, but no protective motor output is implemented yet.
