...
The source and deterministic tests are now joined by a full-muscle bounded physical execution. At NumiBrain `e44009a` and isolated NumanX `949821f`, one process passed the exact ordered private 416-channel NumiBrain allocation into the NumanX MyoSim command buffer, started all activations at zero, integrated the full-body Core candidate, transduced the accepted passive-load peak from source tendon `215`, changed the next maximum excitation from `0.05` to `0.52`, raised commanded tendon `3` activation and force across accepted candidates, replayed a rejected physical candidate exactly, and committed three accepted substeps. Fresh Apple M4 processes emitted byte-identical JSON; the artifact is in [`evidence/numanx-joint-root-v0.3`](evidence/numanx-joint-root-v0.3/README.md), and the original six-channel proof remains in [v0.2](evidence/numanx-joint-root-v0.2/README.md). The reference still synchronizes separate Metal queues, stages MyoSim diagnostics, integrates articulated state on the CPU, and publishes brain and physical roots sequentially. Only the first six channels use fixture gains; the remaining 410 preserve real source-tendon order with zero neural output gain while retaining their NumanX passive force.
