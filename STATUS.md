...
The source and deterministic tests are now joined by a body-localized source-inhibition execution. At NumiBrain `d956927` and isolated NumanX `5cdee51`, one process reconstructed the exact 416-muscle, 157-body attachment catalog, passed the private excitation allocation into MyoSim, built a commit-bound load frame for tendon `215` on bodies `34` and `41`, selected 37 endpoint-sharing motor channels, marked tendon `215` as the sole overloaded source, and zeroed that channel in the next Metal protective output. Commanded tendon `3` activation and force still rose from rest, the rejected physical candidate replayed exactly, and fresh Apple M4 processes emitted byte-identical JSON. The artifact is in [`evidence/numanx-joint-root-v0.6`](evidence/numanx-joint-root-v0.6/README.md), with v0.5 through v0.2 retained as controls. Tendon `215` has zero fixture neural gain and passive force, so the full-body run proves command inhibition identity, not reduced physical load; a synthetic nonzero-gain test proves exact CPU/Metal selective suppression. The load frame and neighborhood remain Swift sidecars. Separate queues, staged diagnostics, CPU articulated integration, sequential publication, mechanical rather than anatomical route identity, and 410 zero-gain channels remain explicit boundaries.
