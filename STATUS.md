...
The source and deterministic tests are now joined by one bounded physical execution. At NumiBrain `20ca7ff` and isolated NumanX `cfcef81`, one process passed the actual private six-channel NumiBrain allocation into the NumanX MyoSim command buffer, integrated the full-body Core candidate, transduced the accepted peak actuator force from source tendon `3`, changed the next maximum excitation from `0.05` to `0.52`, replayed a rejected physical candidate exactly, and committed three accepted substeps. Two fresh Apple M4 processes emitted identical JSON; the artifact is in [`evidence/numanx-joint-root-v0.2`](evidence/numanx-joint-root-v0.2/README.md). The reference still synchronizes separate Metal queues, stages MyoSim diagnostics, integrates articulated state on the CPU, and publishes brain and physical roots sequentially. The six channels use real `.nhmyo` source-tendon identifiers but fixture gains and only a selected subset of the 416-muscle asset.
