# Versioned cohort dispatch v0.20
...
The v0.20 cohort token program activates seven long-range routes and compiles a version-identified 32-publication ring for each route and agent. Before allocation or upload, the runtime simulates every root-plan publication and delayed read against both bounded and unbounded timestamp histories; it rejects the plan if a bounded ring could hide a still-observable publication. Metal converts the authenticated group-major plan into agent-owned due lists before recurrent execution. Each 64-lane environment threadgroup performs parallel match counts, a deterministic inclusive scan, and canonical scatter. The runtime does not yet construct the source plan on the GPU or execute a complete multi-agent brain tick.
...
3. `compact_cohort_invocations` launches one 64-lane threadgroup per active environment from the third count. Each lane scans one contiguous range of materialized groups, the threadgroup performs a deterministic inclusive scan of lane match counts, and lanes rescan their ranges to scatter a canonical private `NBDueInvocation` span plus count.
...
Passing these gates establishes a deterministic versioned dispatch boundary, private Metal materialization, GPU-generated indirect work consumption and parallel agent-major due-list compaction, independent compact diagnostic state, and independent authoritative routed token, history, and selection state. It does not establish GPU-native plan construction, production throughput, or the complete 96-module graph. The current diagnostic lane and invocation compactor still binary-search canonical group entries; the routed kernel no longer does.
