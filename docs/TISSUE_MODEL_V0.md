# Mesoscale neural tissue model v0.6
...
An immutable `uint8` field stores outgoing local-conduction delay class `d_j` for every source site. V0.6 supports integer delays from 0 through 31 accepted integration steps. The synthetic layered profile assigns 1–4 ms classes to its four strata; these classes test causal delay handling and do not encode measured axon length, myelination, or conduction velocity.
...
`R-bar` and `I-bar` blend the local site with its four clamped boundary neighbors. Neighbor contributions are weighted by their outgoing coupling and viability. The relay time constant and explicit history delay model different effects: `R` is local axonal filtering, while `d` selects a committed past relay generation. Sparse projections read the same authoritative history with their edge-specific delay, so local and long-range paths share one causal transaction boundary. The finite stencil is the v0.6 numerical approximation to short-range lateral interaction. The implementation uses forward Euler with explicit clamping to the normalized state interval.
...
Grid coordinates are normalized, not millimetres. Event noise is a bounded numerical receptor-drive perturbation, not a fit to receptor or neural noise statistics. V0.6 therefore establishes causal GPU-compacted noisy-event, delayed structured neural-field computation with transactional Metal due selection, not measured sensing, conduction velocity, cortical thickness, anatomical connectivity, lesion pathology, or tissue scale.
...
holding one 32-bit count and up to 64 32-bit schedule indices. The default scheduler adds 256 descriptor bytes, two 128-byte private clock generations, a 1,536-byte shared interrupt capacity, 40 shared uniform bytes, a 131,072-byte private invocation capacity, and a 16-byte private result. Graph, event, scheduler, memory, transaction, and input identities are explicit schema-v6 evidence fields.
