# Mesoscale neural tissue model v0.17
...
An immutable `uint8` field stores outgoing local-conduction delay class `d_j` for every source site. The configured delay is `d_j` times the model's nominal base timestep. The v0.17 CPU and Metal paths resolve that interval against accepted physical timestamps and linearly interpolate bracketed relay samples. The synthetic layered profile assigns 1–4 ms classes to its four strata; these classes test causal delay handling and do not encode measured axon length, myelination, or conduction velocity.
